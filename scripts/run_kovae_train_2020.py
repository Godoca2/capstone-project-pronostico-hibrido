...
from src.utils.data_loader import load_era5_kovae, split_temporal
from src.models.kovae import KoVAE

OUT_DIR = Path(ROOT) / 'data' / 'models' / 'kovae_trained_step1'
OUT_DIR.mkdir(parents=True, exist_ok=True)

print('Cargando datos ERA5 KoVAE...')
data = load_era5_kovae()
# Se asume clave 'precip_2020' contiene array (T, lat, lon, 1)
...
# Entrenamiento corto (step 1)
epochs = 10
batch_size = 8
res = kovae.train_sequence(X_train, X_val_seq=X_val, epochs=epochs, batch_size=batch_size, patience=5, learning_rate=1e-4)
...
print('Entrenamiento completo. Model saved to', OUT_DIR)
